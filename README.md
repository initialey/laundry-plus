...
- **Slot Capacity Override**(Rider Management パネル内): 日付ごとに1スロットあたりの上限数を手動で直接指定できる(ライダー人数×1ライダーあたり枠数の自動計算を上書き)。未設定の日はこれまで通り自動計算のデフォルトを使用。Set で設定・Clear で解除、値は `Riders` シートの4列目(Capacity Override)に保存
